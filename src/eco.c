...
{
  /* Bona 12/04/2003
     Inserted size and pos controls */
  int f,r, size, pos;
  char next;
    ...
    while ((f<8) && (pos< (size-1))) {
      ...
      if (isalpha(next))
        ...
        if (next != '/') {
			...
			while ((--t) && (f<8));
			// gabrielsan: 26/01/2004
			// BUGFIX:
			// 	sometimes t is negative; therefore, it needs to check
			// 	f to avoid boundary violation
		}
    }
  }
  if (pos < (size-1)) {
	  if (*(FENpos + pos + 1) == 'w') /* the char after the space */
		  gs->onMove = WHITE;
	  else
		  gs->onMove = BLACK;
  }
...
static void ECO_init()
{
  ...
  if (!fp) {
    d_printf( _("Could not open ECO file\n"));
    exit(1);
  }
  while (!feof(fp)) {
    strcpy(ptmp, "");
    ...
    if (ECO_book[i]==NULL) {
      d_printf( _("Could not alloc mem for ECO entry %d.\n"), i);
      exit(1);
    }
  ...
  while (--i >= 0)
    if (ECO_book[i] == NULL)
      d_printf( _("ERROR!  ECO book position number %d is NULL."), i);
}
...
static void NIC_init()
{
  ...
  if (!fp) {
    d_printf( _("Could not open NIC file\n"));
    exit(1);
  }
  while (!feof(fp)) {
    strcpy(ptmp, "");
    ...
    if (NIC_book[i]==NULL) {
      d_printf( _("Could not alloc mem for NIC entry %d.\n"), i);
      exit(1);
    }
...
static void LONG_init()
{
  ...
  if (!fp) {
    d_printf( _("Could not open LONG file\n"));
    exit(1);
  }
  while (!feof(fp)) {
    strcpy(ptmp, "");
    ...
    if (LONG_book[i]==NULL) {
      d_printf( _("Could not alloc mem for LONG entry %d.\n"), i);
      exit(1);
    }
...
void book_open(void)
{
	...
	NIC_init();
	LONG_init();
	d_printf(_("CHESSD: Loaded books\n"));
}
...
char *getECO(int g)
{
  ...
  for (flag=0,i=game_globals.garray[g].numHalfMoves; (i>0 && !flag); i--) {
    ...
    while ((r >= l) && !flag) {
      x = (l+r)/2;
      if ((strcmp((char*) game_globals.garray[g].moveList[i].FENpos, (char*) ECO_book[x]->FENpos)) < 0)
	r = x - 1;
      else
	l = x + 1;
      if (!strcmp((char*) game_globals.garray[g].moveList[i].FENpos, (char*) ECO_book[x]->FENpos)) {
...
int com_eco(int p, param_list param)
{
	...
	if (game_globals.garray[g1].status == GAME_SETUP) {
		pprintf (p,_("The position is still being set up.\n"));
		return COM_OK;
	}
	...
	if (game_globals.garray[g1].private &&
	    ...
	    !check_admin(p, 1)) {
		pprintf(p, _("Sorry - that game is private.\n"));
		return COM_OK;
	} else {
		if (game_globals.garray[g1].type == TYPE_WILD) {
			pprintf(p, _("That game is a wild game.\n"));
			return COM_OK;
		}
	}

	pprintf(p, _("Info about game %d: \"%s vs. %s\"\n\n"), g1+1,
	...
	for (flag=0,i=game_globals.garray[g1].numHalfMoves; (i>0 && !flag); i--) {
		...
		while ((r >= l) && !flag) {
			x = (l+r)/2;
			if ((strcmp((char*) game_globals.garray[g1].moveList[i].FENpos,
                                    (char*) ECO_book[x]->FENpos)) < 0)
				r = x - 1;
			else
				l = x + 1;
			if (!strcmp((char*) game_globals.garray[g1].moveList[i].FENpos,
                                    (char*) ECO_book[x]->FENpos)) {
				pprintf(p, "  ECO[%3d]: %s\n", i, ECO_book[x]->ECO);
				flag=1;
			}
		}
	...
	for (flag=0, i=game_globals.garray[g1].numHalfMoves; ((i>0) && (!flag)); i--) {
		...
		while ((r >=l) && !flag) {
			x = (l+r)/2;
			if ((strcmp((char*) game_globals.garray[g1].moveList[i].FENpos,
                                    (char*) NIC_book[x]->FENpos)) < 0)
				r = x - 1;
			else
				l = x + 1;
			if (!strcmp((char*) game_globals.garray[g1].moveList[i].FENpos,
                                    (char*) NIC_book[x]->FENpos)) {
				pprintf(p, "  NIC[%3d]: %s\n", i, NIC_book[x]->NIC);
				flag=1;
			}
		}
	...
	for (flag=0, i=game_globals.garray[g1].numHalfMoves; ((i>0) && (!flag)); i--) {
		...
		while ((r >=l) && !flag) {
			x = (l+r)/2;
			if ((strcmp((char*) game_globals.garray[g1].moveList[i].FENpos,
                                    (char*) LONG_book[x]->FENpos)) < 0)
				r = x - 1;
			else
				l = x + 1;
			if (!strcmp((char*) game_globals.garray[g1].moveList[i].FENpos,
                                    (char*) LONG_book[x]->FENpos)) {
				pprintf(p, " LONG[%3d]: %s\n", i, LONG_book[x]->LONG);
				flag=1;
			}
		}
