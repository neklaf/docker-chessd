...
void output_shut_mess(void)
{
	...
	d_printf( _("CHESSD: Shutting down at %s\n"), strltime(&shuttime));
}
...
static void ShutDown(void)
{
	...
	TerminateCleanup();
	d_printf( _("CHESSD: Shut down ordered at %s by %s.\n"),
...
void ShutHeartBeat(void)
{
  ...
  if (crossing) {
    d_printf(
    _("CHESSD:   **** Server going down in %d minutes and %d seconds. ****\n\n"),
            timeLeft / 60,
            timeLeft - ((timeLeft / 60) * 60));
    if (reason[0] != '\0')
      d_printf(_("CHESSD: We are going down because: %s.\n"),reason);
    for (p1 = 0; p1 < player_globals.p_num; p1++) {
      if (player_globals.parray[p1].status != PLAYER_PROMPT)
        continue;
      pprintf(p1,
...
int com_shutdown(int p, param_list param)
{
  ...
  if (param[0].type == TYPE_NULL)
    ...
    if (!strcmp(param[0].val.word, "now"))
      ...
      while (*ptr) {
        ...
        default:
          shutdownTime = 0;
          pprintf(p, _("I don't know what you mean by %s\n"), param[0].val.word);
          return COM_OK;
        }
        ptr++;
      }
  ...
  for (p1 = 0; p1 < player_globals.p_num; p1++) {
    ...
    pprintf(p1, _("\n\n    **** Server shutdown ordered by %s. ****\n"), downer);
    if (reason[0] != '\0')
       pprintf(p1, _("    **** We are going down because: %s. ****\n"),reason);
    pprintf(p1,
        _("    **** Server going down in %d minutes and %d seconds. ****\n"),
...

{
 if (check_and_print_shutdown(p) == 0) {
   pprintf (p,_("No shutdown currently in progress\n"));
 }
 return COM_OK;
}
