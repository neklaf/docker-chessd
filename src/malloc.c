...
void m_malloc_check(void *ptr)
{
	...
	if (h->magic != MAGIC1) {
		d_printf(_("malloc corruption ptr=%p v1=%x size=%d\n"),
			 ptr, h->magic, h->size);
	}
}
...
int m_free(void *ptr)
{
	...
	m = h->m;
	if (!m || m->ptr != h)
		d_printf(_("malloc list corruption!\n"));
...
{
	if (!*ptr) {
		d_printf(_("NULL free at %s(%u)!\n"), file, line);
		return;
	}
	if (m_free(*ptr) != 0)
		d_printf(_("bad free at %s(%u)!\n"), file, line);
...
int m_vasprintf(char **strp, const char *fmt, va_list ap)
{
	...
	(*strp) = NULL;
	va_copy(aq,ap); // need to make a copy rather than reuse 'ap'
