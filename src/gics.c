...
/*
 * gabrielsan: 15/01/2004
 * 		All the functions in this file are not needed anymore. User
 * 		stats won't be saved in a file
 *
 */
...
void save_userstat(void)
{
	...
	fprintf(fp, "%d\n%d\n%d\n",
		gics_globals.userstat.usermax,
		gics_globals.userstat.usermaxtime,
		gics_globals.userstat.logins);

	fprintf(fp, "%d\n%d\n%d\n",
		gics_globals.userstat.games,
		gics_globals.userstat.gamemax,
		gics_globals.userstat.gamemaxtime);
...
void game_save_playerratio(char *file, char *Opponent, int Result, int rated)
{
  ...
  if ((fp = fopen_s(file, "r")) != NULL) {
    ...
    if (rename(file1, file) != 0) {
      d_printf(_("CHESSD: Problems rename %s to %s : %d\n"), file1, file, errno);
      return;
    }
    return;
  } else {
    ...
    if(fp == NULL) {
	perror("fopen");
	d_printf(_("Unable to open %s - very bad, info not saved\n"),file);
	return;
    }
    win = loss = draw = 0;
    if (Result == 1)
...
int com_pstat(int p, param_list param)
{
  ...
  if (param[0].type == TYPE_NULL) {
    ...
    if (game_globals.garray[pp->game].status == GAME_EXAMINE) {
      pprintf(p, _("You are examining a game please specify variables.\n"));
      return COM_OK;
    }
    p2 = pp->opponent;
  } else {
  ...
  if (p1 == p2) {
    pprintf(p, _("You have not played against yourself ;-)\n"));
    if (!p1_connected)
      player_remove(p1);
    if (!p2_connected)
      player_remove(p2);
  ...
  if (!flag)
    win = loss = draw = 0;
  pprintf(p, _("Record for %s vs. %s:\n"), player_globals.parray[p1].name, player_globals.parray[p2].name);
  pprintf(p, _("                            wins     losses     draws\n"));
  pprintf(p, _("                rated       %4d       %4d      %4d\n"), win % 100000, loss % 100000, draw % 100000);
  pprintf(p, _("              unrated       %4d       %4d      %4d\n"), win / 100000, loss / 100000, draw / 100000);
